...
(function (window, document, THREE) {
  ...
  var parseNumericArray = function parseNumericArray(string) {
    return string.trim().substr(1, string.length - 2).split(",").map(function (element) {
      return parseFloat(element);
    });
  };

  var parseVector = function parseVector(string) {
    var array = parseNumericArray(string);
    return new THREE.Vector3(array[0], array[1], array[2]);
  };

  var parseQuaternion = function parseQuaternion(string) {
    var array = parseNumericArray(string);
    return new THREE.Quaternion(array[1], array[2], array[3], array[0]);
  };

  var parseGeometry = function parseGeometry(string) {
    var match = null;
  ...
  animate();
}).call(undefined, window, document, THREE);
